...
// i18n.ts 导出相同的 SITE_ORIGIN；为避免 jiti 反向读取 i18n 模块的副作用（syncDocumentLanguage 在浏览器外跳过实现安全）
// 这里直接定义字符串常量并在注释标明来源。如修改 origin，请同步 i18n.ts。
const SITE_ORIGIN = "https://olivesxxxx.github.io/imagex";
...
function buildJsonLd(seo, langName, dates) {
  ...
  const webApp = {
    ...
    sameAs: ["https://github.com/Olivesxxxx/imagex"],
    author: {
      "@type": "Organization",
      name: "ImageX contributors",
      url: "https://github.com/Olivesxxxx/imagex",
      sameAs: ["https://github.com/Olivesxxxx/imagex"],
    },
    publisher: {
      "@type": "Organization",
  ...
  const breadcrumb = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "ImageX", item: seoBaseUrl },
